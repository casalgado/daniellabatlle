class Spree::Admin::WholesalersController < Spree::Admin::ResourceController
  ...
  def create
    ...
    if @wholesaler.save
      flash[:notice] = Spree.t('spree.admin.wholesaler.success')
      redirect_to spree.admin_wholesalers_path
    else
      flash[:error] = Spree.t('spree.admin.wholesaler.failed')
      render :action => "new"
    end
  end
  ...
  def update
    ...
    if @wholesaler.update_attributes(params[:wholesaler])
      flash[:notice] = Spree.t('spree.admin.wholesaler.update_success')
    else
      flash[:error] = Spree.t('spree.admin.wholesaler.update_failed')
    end
    respond_with(@wholesaler)
  end
  ...
  def destroy
    @wholesaler = Spree::Wholesaler.find(params[:id])
    @wholesaler.destroy
    flash[:notice] = Spree.t('spree.admin.wholesaler.destroy_success')
    respond_with(@wholesaler)
  end
